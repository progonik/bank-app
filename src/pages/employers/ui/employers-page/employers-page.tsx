...
import {
  useHrsQuery,
  createEmployersColumns,
  FILTER_ALL,
  PAGE_SIZES,
  DEFAULT_PAGE_SIZE,
  SCROLL_HEIGHT,
} from "../../model";
import { EmployersRowActions } from "../employers-row-actions";
import { AddEmployerDialogPlaceholder } from "../add-employer-dialog-placeholder";
import { EmployersFiltersBar } from "../employers-filters-bar";
...
export function EmployersPage() {
  ...
  const rightSideActions = useMemo(
    () => (
      <ColumnVisibilityFilter<HrApi>
        columns={allColumns}
        visibleColumns={visibleColumns}
        onVisibilityChange={setVisibleColumns}
        label={t("employers.columnVisibility")}
        getColumnLabel={(col) => t(`employers.columns.${col.key}`)}
      />
    ),
  ...
  const renderActions = useCallback(
    (hr: HrApi) => <EmployersRowActions hr={hr} />,
    []
  );
  ...
  const headerAction = useMemo(
    () => ({
      label: t("employers.addEmployer"),
      open: addDialogOpen,
      onOpenChange: setAddDialogOpen,
      dialogContent: <AddEmployerDialogPlaceholder t={t} />,
    }),
    [t, addDialogOpen]
  );
  ...
  if (isError) {
    return (
      <div className="p-8 flex flex-col gap-4">
        <PageTableHeader title={t("employers.title")} />
